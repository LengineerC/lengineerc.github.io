...
export default function Post({ id, initialContent }: { id: string; initialContent?: PostContent }) {
  const [loadedContent, setLoadedContent] = useState<LoadedPostContent | undefined>(() =>
    initialContent ? { path: '', value: initialContent } : undefined,
  );
  ...
  const content =
    loadedContent && (loadedContent.path === postConfig?.contentPath || loadedContent.path === '')
      ? loadedContent.value
      : undefined;
  ...
  useEffect(() => {
    ...
    if (initialContent) {
      if (loadedContent?.path !== contentPath) {
  ...
  }, [id, initialContent, loadedContent?.path, locked, navigate, postConfig]);
