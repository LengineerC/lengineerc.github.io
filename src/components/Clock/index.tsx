...
export default function Clock(props: Props) {
  ...
  const initCanvas = () => {
    ...
    if (dialPlateRef.current) {
      dialPlateRef.current.width = width * dprRef.current;
      dialPlateRef.current.height = height * dprRef.current;
      dialPlateRef.current.style.width = `${width}px`;
      dialPlateRef.current.style.height = `${height}px`;

      dialPlateCtxRef.current?.scale(dprRef.current, dprRef.current);
    }

    if (handsRef.current) {
      handsRef.current.width = width * dprRef.current;
      handsRef.current.height = height * dprRef.current;
      handsRef.current.style.width = `${width}px`;
      handsRef.current.style.height = `${height}px`;

      handsCtxRef.current?.scale(dprRef.current, dprRef.current);
    }
  }
