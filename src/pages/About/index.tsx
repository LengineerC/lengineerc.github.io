...
export default function About() {
  ...
  const categoriesList = useAppSelector(state => state.taxonomy.categoriesList) as any;

  const getFormatCategoriesData = () => {
    let data: any[] = [];
    // const {categoriesList}=store.getState();

    if (categoriesList && typeof categoriesList === 'object') {
      data = Object.keys(categoriesList).map(key => {
        return ({
          name: key,
          value: Array.isArray(categoriesList[key]) ? categoriesList[key].length : 0,
        })
      });
    }
    return data;
        ...
        <Card darkMode={darkMode}>
          <div className="about-main">
            <div className="about-avatar">
              <img src={typeof avatar === 'string' ? avatar : avatar.src} alt="avatar" />
            </div>
