...
export default function Archives() {
  ...
  const githubRepoCommitsFromStore = useAppSelector(state => state.app.githubRepoCommits);

  useEffect(() => {
    // 如果 store 中没有数据，则发起请求
    if (!githubRepoCommitsFromStore || githubRepoCommitsFromStore.length === 0) {
      dispatch({
  ...
  useEffect(() => {
    // 当 store 中的数据更新时，格式化并显示
    if (githubRepoCommitsFromStore && githubRepoCommitsFromStore.length > 0) {
      formatCommitData(githubRepoCommitsFromStore);
    }
  }, [githubRepoCommitsFromStore])
  ...
  }, [darkMode, postList, heatMapRef.current]);
